...
A full-stack fitness tracking web app that brings together everything you need in one place: workout logging, nutrition tracking, hydration, body metrics, and goals.

Live demo: [fit-track-sigma-woad.vercel.app](https://fit-track-sigma-woad.vercel.app)
...
This creates the `fittrack` database, all the tables, and seeds the exercise library (15 exercises) and food library (15 foods with nutrient profiles).
...
Then open http://localhost:3001 in your browser and you're good to go.
...
| `POST` | `/nutrition/meals/:meal_id/foods` | Add a food item to a meal |
| `DELETE` | `/nutrition/meals/:meal_id/foods/:food_name` | Remove a food item from a meal |
...
The schema comes pre-loaded with a starter set of exercises and foods so you can hit the ground running.
...
| `DB_NAME` | `fittrack` | Database name |
